	...
	case *ast.CallExpr:
		...
		case *ast.SelectorExpr:
			...
			if v, ok := workspace[k]; ok {
				...
				case "check":
					if len(ex.Args) != 0 {
						return errors.New("check() expects no arguments")
					}
					kept := v.Dedup()
					for i := 0; i < len(kept); i++ {
						for j := i + 1; j < len(kept); j++ {
							if frame.HasDeadLock(kept[i], kept[j]) {
								sgr.Println("================= [fg-red]WARNING DEAD LOCK [reset]=================\n")
								sgr.Printf("goroutine [fg-blue]%d[reset] has surspicous DEAD LOCK with goroutine [fg-blue]%d[reset]\n", kept[i].Id, kept[j].Id)
								sgr.Printf("LockHolders of goroutine %d: [fg-green]%v[reset]\n", kept[i].Id, kept[i].LockHolders)
								sgr.Printf("LockHolders of goroutine %d: [fg-green]%v[reset]\n", kept[j].Id, kept[j].LockHolders)
							}
						}
					}
					return nil
